...
Objective: Run the following R code in RStudio or Positron to demonstrate how to create objects and determine the class of different vectors using the `class()` function. This exercise will help you understand the fundamental data types in R and how they are used in data analysis.
...
These are the same in RStudio and Positron. In Positron you can also reach any
command by name through the command palette, `Ctrl/Cmd + Shift + P`.
...
- **CTRL + SHIFT + M / CMD + SHIFT + M**: Creates a pipe symbol. Which pipe you get depends on a setting: in RStudio it is *Tools → Global Options → Code → Use native pipe operator*; in Positron it is the `positron.r.pipe` setting.
