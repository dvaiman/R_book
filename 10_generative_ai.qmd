...
@kahneman2011 described two modes of thinking. **System 1** is fast, automatic and
...
@shaw2025 call this **"System 3"** — artificial cognition happening outside the
head — and warn about *cognitive surrender*: accepting AI output with minimal
scrutiny. They measured it in pre-registered experiments with 1,372
...
Research on learning [@bjork2011] shows that some difficulties feel slow and
...
|---|---|
| **80%** of developers use AI, but only **29%** trust its correctness [@stackoverflow2025] | Using without trusting: **verification is the norm** |
| The most common frustration (45%): code that is **"almost right"**; 66% spend more time fixing it | The jagged edge, in survey form |
| Developers *believed* they were 20% faster with AI. They were **19% slower** [@metr2025] | The System 1 feeling of progress misleads |
...
In 1985 Peter Naur argued that a program is not its source code [@naur1985]. The program is
...
An agent does best with a small, well-defined task, and you keep ownership
most easily when you can see the whole task at once. A workflow that suits
research code: **ask → explore → plan → implement → test → review**. Let the
agent explore the project and propose a plan, and read the plan *before* it
changes anything.

A good task has five parts:

```text
Goal:         Add a check for missing values to R/02_clean.R.
Context:      Read R/01_import.R and data-dict.yaml first.
Constraints:  Do not change any other file. Use the tidyverse and |>.
Done when:    The script stops if a required column has missing values.
Verification: Show the plan first. Then run it on data/sim.csv and show the result.
```

Notice two things. The data dictionary (@sec-data-dictionary) is named as
context, so the agent knows what "required" means for each column without
seeing the data. And the agent runs on **simulated** data, not the real data.
The *done when* line is your definition of correct, written before the code
exists.

Prefer **small, testable steps** over "rewrite the whole analysis". When
something goes wrong, you can see where. (Workflow after Metwalli,
...
dictionary**, what registry research calls a **codebook**: one document that
records the grain, the types, the units, the allowed values and what missing
means. The same document serves your collaborators, your reviewers, your future
self and an AI tool, and you share the *description* rather than the rows.
...
→ **standardise** it machine-readably as a `data-dict.yaml` that the data can
be validated against.

@sec-data-dictionary covers this properly: the `data-dict` format, drafting a
dictionary from the data in R, checking the data against it, and which fields
(`range`, `values`, `examples`) have to be generalised before a dictionary of
...
## Understand what the plot shows

AI makes good-looking plots quickly. It is still **you** who has to know what
they show. Ask three questions of every plot:

- What **question** should the plot answer, and does the plot type fit it?
- What is one point or bar: an individual, a measurement, or a **mean**?
- What is **hidden**: the spread, the number of observations, rows dropped
  because of missing values, an axis that does not start at zero?

Ask for "mean highway mileage per car class" and you may get the plot on the
left. It is not wrong, but it says less than it seems to.

```{r}
#| label: ai-plot-mean
#| fig-height: 3.5
#| fig-cap: "What AI suggested: mean highway mileage per class."
mpg |>
...
```{r}
#| label: ai-plot-points
#| fig-height: 3.5
#| fig-cap: "The same data, with every car shown."
mpg |>
  ggplot(aes(x = class, y = hwy)) +
...
With every car visible, you can see that the `2seater` bar rests on only five
cars, and that the spread within `compact`, `midsize` and `subcompact` is much
larger than the differences between their means. With repeated measurements per person, "what is one
point?" matters even more: a mean of means and a mean of all measurements are
different numbers. And `ggplot2` drops rows with missing values with only a
warning. You own the conclusion the plot leads to, even if AI wrote the code
(@sec-visualisation).
...
```{r}
#| label: fig-horst-debugging
#| echo: false
#| out.width: "70%"
#| fig-cap: |
#|   The emotional stages of debugging. Artwork by Allison Horst, CC BY 4.0.
#| fig-alt: |
#|   Ten faces of a monster debugging, from I got this through meltdown to a new hope and I love coding.
knitr::include_graphics("images/horst/debugging.jpg")
```
...
- [How to work with AI coding agents](https://towardsdatascience.com/how-to-work-with-ai-coding-agents/)
  — Metwalli on the workflow and task structure for coding agents
...
- [10 tips for getting better R code from your AI coding agent](https://www.infoworld.com/article/4184642/10-tips-for-getting-better-r-code-from-your-ai-coding-agent.html)
